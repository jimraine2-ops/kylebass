...
const PENNY_ENTRY_SCORE = 65; // ★ 동전주 진입 문턱: 65점 (복리 전략: 진입 기회 확대)
const PENNY_BREAKEVEN_PCT = 1.0; // ★ 동전주 본절보호: +1.0%
const PENNY_IRON_HOLD_SCORE = 60; // ★ 동전주 철갑 홀딩: 60점 (복리: 약간 완화)
const PENNY_MAX_POSITIONS = 3; // ★ 동전주 최대 3종목 (실제 적용)
const GHOST_BREAKEVEN_PCT = 1.0; // ★ Zero-Risk Lock: +1.0% 돌파 시 SL→매수가+0.3%
const PROFIT_CHASE_TRIGGER = 3.0; // ★ 3% 익절 확정 트리거 (복리 기본 수익 확보선)
const PROFIT_CHASE_SL_PCT = 1.015; // ★ 수익 추격 SL: 매수가 +1.5%
const TRAILING_DROP_PCT = 1.5; // ★ 추격 매도: 고점 대비 1.5% 하락 (복리: 수익 보호 강화)
const DAILY_TARGET_KRW_MIN = 300000; // ★ [복리매매] 일일 최소 목표: ₩300,000
const DAILY_TARGET_KRW_MAX = 500000; // ★ [복리매매] 일일 최대 목표: ₩500,000
const ZERO_RISK_SL_PCT = 1.003; // ★ Iron-Defense 1단계 SL: 매수가 +0.3% (복리: 수익 보호)
...
// ★ [복리매매] 라운드 추적
let currentRound = 1;
let roundResetTimestamps: string[] = [];
let cumulativeTotalProfitKRW = 0; // 누적 수익 (복리 원금에 포함)

// ★ [복리매매] 일일 목표 동적 계산: 현재 잔고 기반 6~10% 목표
function getDailyTarget(currentBalance: number): { min: number; max: number; label: string } {
  const dynamicMin = Math.max(DAILY_TARGET_KRW_MIN, Math.round(currentBalance * 0.06));
  const dynamicMax = Math.max(DAILY_TARGET_KRW_MAX, Math.round(currentBalance * 0.10));
  return { min: dynamicMin, max: dynamicMax, label: `₩${dynamicMin.toLocaleString()}~₩${dynamicMax.toLocaleString()}` };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ========== [INFINITE LOOP] 수익 목표 달성 시 자동 리셋 & 재공략 ==========
    {
      ...
      const dailyTarget = getDailyTarget(balance);
      
      if (currentRoundPnl >= dailyTarget.min && currentOpenCount === 0) {
        // ★ [복리매매] 최소 목표 ₩300k 달성 + 포지션 청산 → 복리 재투자 라운드
        cumulativeTotalProfitKRW = todayRealizedPnl;
        const profitSaved = currentRoundPnl;
        const completedRound = currentRound;
        currentRound++;
        roundResetTimestamps.push(now.toISOString());
        
        // ★ [복리매매 핵심] 수익금을 원금에 합산하여 다음 라운드 투자 (리셋하지 않음!)
        // 현재 잔고 = 원금 + 수익금 → 그대로 유지 (복리 효과)
        const compoundBalance = balance; // 복리: 잔고 유지 (수익금 재투입)
        
        await addLog('system', 'milestone', null, 
          `🏆📈 [복리매매 Round ${completedRound} 완료 → Round ${currentRound}!] ` +
          `목표 ${dailyTarget.label} 달성! (실현: ${fmtKRWRaw(profitSaved)}) | ` +
          `💰 복리 잔고: ${fmtKRWRaw(compoundBalance)} (수익금 재투입!) | ` +
          `누적 수익: ${fmtKRWRaw(cumulativeTotalProfitKRW)} | ` +
          `다음 라운드 목표: ${getDailyTarget(compoundBalance).label}`,
          { 
            completedRound, newRound: currentRound, 
            roundProfit: profitSaved, cumulativeProfit: cumulativeTotalProfitKRW,
            compoundBalance, strategy: 'compound',
        ...
        await addLog('unified', 'info', null, 
          `[복리 Round ${currentRound}] 🎯 복리매매 엔진 재부팅 — ` +
          `잔고 ${fmtKRWRaw(compoundBalance)} 전액 재투입 | ` +
          `복리 효과: 수익↑ → 포지션 크기↑ → 수익 가속! | ` +
          `다음 목표: ${getDailyTarget(compoundBalance).label}`,
          { round: currentRound, balance: compoundBalance }
        );
      } else if (currentRoundPnl >= dailyTarget.min && currentOpenCount > 0) {
        await addLog('system', 'info', null, 
          `[복리 Round ${currentRound}] 💰 최소 목표 ${fmtKRWRaw(dailyTarget.min)} 달성! ` +
          `(현 수익: ${fmtKRWRaw(currentRoundPnl)}) | ` +
          `잔여 ${currentOpenCount}개 청산 대기 → 최대 목표 ${fmtKRWRaw(dailyTarget.max)} 추격 중`,
    ...
    for (const sym of symbolsToCheck) {
      ...
      for (const pos of (openPos || []).filter((p: any) => p.symbol === sym && p.status === 'open')) {
        ...
        if (!shouldClose && pos.stop_loss && price <= pos.stop_loss) {
          ...
          } else if (pnlPct > -5 && quantScore >= 50) {
            // ★ [복리] -5% 이내 + 50점 이상: 홀딩
            await addLog('unified', 'hold', sym, `[복리홀딩] ${sym} -${Math.abs(pnlPct).toFixed(2)}% | 지표 ${quantScore}점(≥50) → 반등 대기`, { quantScore, metCount, pnlPct: +pnlPct.toFixed(2), coreIntact, isIronHold });
          } else if (pnlPct > -5 && quantScore >= 40) {
            // ★ [복리] 40~49점: 경고
            await addLog('unified', 'warning', sym, `[복리주의] ${sym} -${Math.abs(pnlPct).toFixed(2)}% + 지표 ${quantScore}점(40~49) → 매도 유보`, { quantScore, pnlPct: +pnlPct.toFixed(2) });
          } else if (pnlPct <= -5 && quantScore >= 55 && (coreIntact || technicalSafe)) {
            // ★ [복리] -5% 이하: 55점 이상 + 기술 안전 → 예외적 홀딩 (엄격화)
            await addLog('unified', 'hold', sym, `[복리예외홀딩] ${sym} -${Math.abs(pnlPct).toFixed(2)}% BUT 지표 ${quantScore}점(≥55) + ${coreIntact ? 'VWAP+이평선 정배열' : '기술안전'} → 예외 홀딩`, { quantScore, metCount, pnlPct: +pnlPct.toFixed(2), coreIntact, isIronHold });
          } else if (pnlPct <= -5 && quantScore >= 40) {
            await addLog('unified', 'warning', sym, `[⚠️ 복리 손절 경고] ${sym} -${Math.abs(pnlPct).toFixed(2)}% -5% 이하 + 지표 ${quantScore}점(40~49) → 매도 임박`, { quantScore, pnlPct: +pnlPct.toFixed(2) });
          } else {
    ...
    // ========== [복리매매] 일일 수익 목표 체크 (₩300,000~₩500,000 동적) ==========
    const todayStart = new Date(now);
    todayStart.setHours(0, 0, 0, 0);
    const { data: todayClosedTrades } = await supabase
      .from('unified_trades')
    ...
    const dailyTarget = getDailyTarget(balance);
    const dailyMinHit = dailyPnl >= dailyTarget.min;
    const dailyMaxHit = dailyPnl >= dailyTarget.max;
    
    // ★ [복리매매] 일일 최대 손실 방어: 잔고의 10% 이상 손실 시 매수 중단
    const dailyLossLimit = Math.round(balance * MAX_DAILY_LOSS_PCT);
    const isDailyLossExceeded = dailyPnl < 0 && Math.abs(dailyPnl) >= dailyLossLimit;
    
    if (dailyMaxHit) {
      await addLog('unified', 'milestone', null, `🎉🏆 [복리매매 최대 목표 달성!] 실현 수익 ${fmtKRWRaw(dailyPnl)} ≥ ${fmtKRWRaw(dailyTarget.max)} | 복리 잔고: ${fmtKRWRaw(balance)}`, { dailyPnl, target: dailyTarget });
    } else if (dailyMinHit) {
      await addLog('unified', 'milestone', null, `💰 [복리매매 최소 목표 달성!] 실현 수익 ${fmtKRWRaw(dailyPnl)} ≥ ${fmtKRWRaw(dailyTarget.min)} | 최대 ${fmtKRWRaw(dailyTarget.max)} 추격 중`, { dailyPnl, target: dailyTarget });
    } else if (isDailyLossExceeded) {
      await addLog('system', 'warning', null, `🚨 [복리 손실 방어] 일일 손실 ${fmtKRWRaw(Math.abs(dailyPnl))} ≥ 잔고의 10%(${fmtKRWRaw(dailyLossLimit)}) → 신규 매수 중단!`, { dailyPnl, lossLimit: dailyLossLimit });
    } else {
      const progressPct = dailyTarget.min > 0 ? (dailyPnl / dailyTarget.min * 100) : 0;
      await addLog('system', 'info', null, `[복리목표] PnL: ${fmtKRWRaw(dailyPnl)} / 목표 ${dailyTarget.label} (${progressPct.toFixed(1)}%) | 복리 잔고: ${fmtKRWRaw(balance)}`, { dailyPnl, target: dailyTarget, balance });
    }
    ...
    // ★ [복리매매 Day-Break] KST 9:00 → 매수 제한 해제, 복리 잔고 유지 (리셋 없음!)
    const isDayBreakWindow = kstTimeMinutes >= 540 && kstTimeMinutes < 545; // KST 9:00~9:05
    if (isDayBreakWindow && currentOpenCountForPause === 0) {
      // ★ [복리매매] 잔고 리셋 금지! 현재 잔고 그대로 복리 운용
      const compoundTarget = getDailyTarget(balance);
      
      await addLog('system', 'day-break', null, 
        `[복리 Day-Break] 🟢 Round ${currentRound} 복리매매 재개! | ` +
        `복리 잔고: ${fmtKRWRaw(balance)} (리셋 없음 — 수익금 재투입!) | ` +
        `오늘 목표: ${compoundTarget.label} | ` +
        `₩12,000↓ 저가주 + 익절확률 85%↑ 종목 즉시 투입`,
        { dayBreak: true, round: currentRound, balance, dailyPnl, target: compoundTarget }
      );
    }
    
    // ★ [복리매매] 방어 체계 상시 가동 로그
    await addLog('system', 'defense', null, 
      `[복리매매] 🛡️ 방어 체계 상시 가동 | SL -5%(엄격) | +1%→SL+0.3% 본절 | +3%→즉시 확정 | 일일 손실 한도 잔고 10% | 체결강도 200%↑→트레일링(-1.5%)`,
      { compound: true }
    );

    // ★ [데이장 단타] 정규장 진입 시 데이장 매수 종목 자동 매도 검토
    if (currentSession === 'REGULAR') {
      for (const pos of (openPos || []).filter((p: any) => p.status === 'open')) {
        const isDayTradeEntry = pos.ai_reason?.includes('선취매') || 
          pos.ai_reason?.includes('PRE') || 
          pos.ai_reason?.includes('OVERNIGHT') ||
          pos.ai_reason?.includes('데이장') ||
          pos.ai_reason?.includes('DAY');
        
        if (!isDayTradeEntry) continue;
        
        const dtData = await getQuoteAndCandles(pos.symbol);
        if (!dtData) continue;
        const dtPrice = dtData.quote.c;
        const dtPnlPct = ((dtPrice - pos.price) / pos.price) * 100;
        const dtScoring = score10Indicators(dtData.quote, dtData.closes, dtData.highs, dtData.lows, dtData.opens, dtData.volumes, false);
        const dtQuantScore = dtScoring?.totalScore || 0;
        const minutesSinceOpen = (etTime - 570);
        
        if (dtPnlPct >= 2.0 && minutesSinceOpen >= 30) {
          const saleProceeds = Math.floor(dtPrice * pos.quantity * KRW_RATE);
          const buyCost = Math.floor(pos.price * pos.quantity * KRW_RATE);
          const pnlKRW = saleProceeds - buyCost;
          const newBalance = balance + saleProceeds;
          
          await supabase.from('unified_trades').update({
            status: 'day_trade_profit', close_price: dtPrice, pnl: pnlKRW,
            closed_at: now.toISOString(),
            ai_reason: `[🌙→☀️ 데이장 단타 익절] ${pos.symbol} +${dtPnlPct.toFixed(1)}% | PnL: ${fmtKRWRaw(pnlKRW)}`,
          }).eq('id', pos.id);
          await supabase.from('unified_wallet').update({ balance: newBalance, updated_at: now.toISOString() }).eq('id', wallet.id);
          balance = newBalance;
          
          await addLog('unified', 'exit', pos.symbol, 
            `[🌙→☀️ 데이장 단타] ${pos.symbol} 프리마켓 매수 → 정규장 +${dtPnlPct.toFixed(1)}% 익절 | PnL: ${fmtKRWRaw(pnlKRW)} | 복리 잔고: ${fmtKRWRaw(newBalance)}`, 
            { pnl: pnlKRW, pnlPct: +dtPnlPct.toFixed(2), strategy: 'day_trade' }
          );
        } else if (dtPnlPct >= 0.5 && minutesSinceOpen >= 60 && dtQuantScore < 55) {
          const saleProceeds = Math.floor(dtPrice * pos.quantity * KRW_RATE);
          const buyCost = Math.floor(pos.price * pos.quantity * KRW_RATE);
          const pnlKRW = saleProceeds - buyCost;
          const newBalance = balance + saleProceeds;
          ...
          await supabase.from('unified_trades').update({
            status: 'day_trade_rotation', close_price: dtPrice, pnl: pnlKRW,
            closed_at: now.toISOString(),
            ai_reason: `[🌙→☀️ 데이장 회전] ${pos.symbol} 지표 ${dtQuantScore}점 → +${dtPnlPct.toFixed(1)}% 회전 | PnL: ${fmtKRWRaw(pnlKRW)}`,
          }).eq('id', pos.id);
          await supabase.from('unified_wallet').update({ balance: newBalance, updated_at: now.toISOString() }).eq('id', wallet.id);
          balance = newBalance;
          
          await addLog('unified', 'exit', pos.symbol, 
            `[🌙→☀️ 데이장 회전] ${pos.symbol} 지표 ${dtQuantScore}점(<55) → +${dtPnlPct.toFixed(1)}% 빠른 회전`,
            { pnl: pnlKRW, strategy: 'day_trade_rotation' }
          );
        } else if (dtPnlPct > 0) {
          await addLog('unified', 'info', pos.symbol, 
            `[🌙 데이장 단타] ${pos.symbol} +${dtPnlPct.toFixed(2)}% | 지표 ${dtQuantScore}점 | 정규장 ${minutesSinceOpen}분 → +2% 대기`,
            { pnlPct: +dtPnlPct.toFixed(2), quantScore: dtQuantScore }
          );
        }
      }
    }
    ...
    // ★ [복리매매] 일일 최대 손실 초과 시 매수 완전 중단
    if (isDailyLossExceeded) {
      await addLog('unified', 'warning', null, `[복리 손실 방어] 🚫 일일 손실 한도(${fmtKRWRaw(dailyLossLimit)}) 초과 → 금일 신규 매수 전면 중단`, { dailyPnl, lossLimit: dailyLossLimit });
    }

    if (!marketBuyHalt && !isOpeningRush && !safePauseActive && !isDailyLossExceeded) {
      for (let i = 0; i < SCAN_SYMBOLS.length; i += 5) {
        ...
        for (const r of results) {
          ...
          // ★ [복리] 동전주 최대 포지션 제한 실제 적용
          if (isPenny && pennyOpenCount >= PENNY_MAX_POSITIONS && !isPyramiding) continue;
    ...
    for (const r of topCandidates) {
      ...
      // ★ [복리매매] 균등 분산 투자: 집중 위험 방지, 복리 수익 안정화
      // 최대 30% → 25% → 20% → 15% 분산 (1종목 과집중 방지)
      const isTopRanked = topCandidates.indexOf(r) === 0;
      const positionPct = isPyramiding ? 0.05 : isTopRanked ? 0.30 : isPennyEntry ? 0.25 : (isCriticalPatternEntry || isSuperEntry || isScoreSurge) ? 0.25 : 0.20;
      ...
      // ★ [복리매매] 초기 SL -5% / TP +7~10% (R:R 1:1.4~2.0 확보)
      const stopLoss = +(adjustedPrice * STRICT_STOP_LOSS_PCT).toFixed(4); // -5% 엄격 손절
      // ★ 선취매/예측형: TP +10%, 일반: +7% (복리: 수익 목표 상향)
      const tpMultiplier = (isAccumEntry || isCriticalPatternEntry || isPredictive) && isLowVolumeSession ? 1.10 : 1.07;
    ...
    if (dipBuyCandidates.length > 0 && openCount < MAX_POSITIONS && !isDailyLossExceeded) {
      ...
      for (const dip of dipBuyCandidates.slice(0, 2)) {
        ...
        const sl = +(ap * STRICT_STOP_LOSS_PCT).toFixed(4); // ★ [복리] -5% 엄격 손절
        const tp = +(ap * 1.07).toFixed(4); // ★ [복리] +7% 목표
